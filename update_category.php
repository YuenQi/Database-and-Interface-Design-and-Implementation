...
if(isset($_POST['submit'])){
    ...
    if(empty($_POST['id'])){
        ...
        $sql_check_id = "SELECT category_id FROM category WHERE category_id = $id_to_update";
    ...
    if(empty($_POST['name'])){
        $errors['name'] = 'Please key in category name. <br />'; 
    }
    else{
        $name = $_POST['name'];
        if(!preg_match('/^[a-zA-Z\s]+$/', $name)){
            $errors['name'] = 'Category name must be letters and/or spaces only.';
        }
    }
    ...
    if(array_filter($errors)){
        ...
        $sql = "UPDATE category SET name = '$name', last_update = '$last_update' WHERE category_id = '$id_to_update'";
...
<body>
    ...
    <form action="#" class="white" method="POST">
        <label>Category Id that you wish to update: </label>
