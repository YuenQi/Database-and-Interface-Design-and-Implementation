...
<body>
    ...
    <h2 class="center"><b>Staff's Email</b></h2>
    ...
    <br>
    <h5>Type in staff's email: </h5>
    <input type="text" id="myInput" onkeyup="myFunction()" placeholder="Search for staff's email.." title="Type in staff's email">
    ...
    <table id="myTable">
        ...
        $query = "SELECT * FROM staff_email";

        if (isset($_POST['order_list'])) {
            $query = "SELECT * FROM staff_email ORDER BY " . $_POST['order_list'];
        }

        $result = mysqli_query($conn, $query);

        while ($row = mysqli_fetch_array($result)) {
            echo '<tr><td>' . $row['email_id'] . '</td><td>' . $row['staff_id'] . '</td><td>' .
                $row['email'] . '</td><td>' . $row['active'] . '</td><td>' . $row['username'] . '</td><td>' .
                $row['password'] . '</td><td>' . $row['last_update'] . "</td></tr>";
        }
        ?>
    </table>
