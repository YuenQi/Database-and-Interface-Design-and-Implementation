...
<body>
    ...
    <h2 class="center"><b>Payment</b></h2>
    ...
    <br>
    <h5>Type in payment ID: </h5>
    <input type="text" id="myInput" onkeyup="myFunction()" placeholder="Search for payment ID..." title="Type in payment ID">
    ...
    <table id="myTable">
        <tr class="header">
            ...
            <th style="width:11%;">Rental ID</th>
        ...
        if (isset($_POST['order_list'])) {
            $query = "SELECT * FROM payment ORDER BY " . $_POST['order_list'] . ", payment_id";
        }

        $result = mysqli_query($conn, $query);

        while ($row = mysqli_fetch_array($result)) {
            echo '<tr><td>' . $row['payment_id'] . '</td><td>' . $row['customer_id'] . '</td><td>' .
                $row['staff_id'] . '</td><td>' . $row['rental_id'] . '</td><td>' . $row['amount'] . '</td><td>' .
                $row['payment_date'] . '</td><td>' . $row['last_update'] . "</td></tr>";
        }
        ?>
    </table>
